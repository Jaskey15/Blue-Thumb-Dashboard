...
for use in the Survey123 sync Cloud Function. It reuses existing functions
to ensure consistency with the main processing pipeline.
"""
...
# Add the parent directory to the path so we can import from data_processing
...
def get_reference_values_from_db(conn):
    """
    Get reference values from the database for cloud function use.
...
def process_survey123_chemical_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This function uses the exact same workflow as updated_chemical_processing.py
    but operates on Survey123 data passed as a DataFrame instead of loading from CSV.
    
    Args:
        df: Raw Survey123 chemical data DataFrame
    ...
    try:
        ...
        # Step 1: Parse dates (reusing existing function)
        df = parse_sampling_dates(df)
        
        # Step 2: Process all nutrients (reusing existing functions)
        ...
        # Step 3: Format to database schema (reusing existing function)
        formatted_df = format_to_database_schema(df)
        
        # Step 4: Clean and validate (reusing existing functions)
        formatted_df = remove_empty_chemical_rows(formatted_df)
        formatted_df = validate_chemical_data(formatted_df, remove_invalid=True)
        
        # Step 5: Apply BDL conversions (reusing existing function)
...
def insert_processed_data_to_db(df: pd.DataFrame, db_path: str) -> Dict[str, Any]:
    ...
    Args:
        df: Processed chemical data DataFrame
    ...
    try:
        ...
        # Get reference values for status determination (using local function)
        ...
        for _, row in df.iterrows():
            ...
            for param_name, param_id in parameter_map.items():
                if param_name in row and pd.notna(row[param_name]):
                    ...
                    # Determine status using existing function
