...
class TestScoreAveraging(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Sample BT field work data with proper REP structure
...
Red River at Bridge,7/12/2023,REP"""
    ...
    def test_categorize_and_process_duplicates_no_bt_data(self):
        ...
        # Create empty BT DataFrame with expected columns
        empty_bt = pd.DataFrame(columns=['Site_Clean', 'Year', 'Is_REP', 'Date_Clean'])
        
        result = categorize_and_process_duplicates(self.sample_fish_data, empty_bt)
        
        # Should process duplicates by averaging (no REP data available)
        self.assertLess(len(result), len(self.sample_fish_data))
    ...
    def test_integration_categorize_vs_averaging(self):
        """Integration test comparing different duplicate processing approaches."""
        ...
        # Process without BT data (should average all duplicates)
        empty_bt = pd.DataFrame(columns=['Site_Clean', 'Year', 'Is_REP', 'Date_Clean'])
        result_without_bt = categorize_and_process_duplicates(self.sample_fish_data, empty_bt)
        ...
        # With BT data: Red River keeps 2 records (replicates), Blue Creek averages to 1
        # Without BT data: Both duplicate groups should be averaged
        self.assertGreaterEqual(len(result_with_bt), len(result_without_bt))

    def test_duplicate_processing_preserves_unique_sites(self):
        """Test that sites without duplicates are preserved unchanged."""
        # Solo Site and Tenmile Creek should be preserved in all scenarios
        result_with_bt = categorize_and_process_duplicates(self.sample_fish_data, self.sample_bt_data)
        empty_bt = pd.DataFrame(columns=['Site_Clean', 'Year', 'Is_REP', 'Date_Clean'])
        result_without_bt = categorize_and_process_duplicates(self.sample_fish_data, empty_bt)
        
        # Both should contain Solo Site and Tenmile Creek unchanged
        for result in [result_with_bt, result_without_bt]:
            solo_site = result[result['site_name'] == 'Solo Site']
            tenmile_creek = result[result['site_name'] == 'Tenmile Creek at Davis']
            
            self.assertEqual(len(solo_site), 1)
            self.assertEqual(len(tenmile_creek), 1)
            
            # Values should be unchanged
            original_solo = self.sample_fish_data[self.sample_fish_data['site_name'] == 'Solo Site'].iloc[0]
            self.assertEqual(solo_site.iloc[0]['comparison_to_reference'], original_solo['comparison_to_reference'])
    ...
    def test_edge_case_single_sample_per_site(self):
        ...
        empty_bt = pd.DataFrame(columns=['Site_Clean', 'Year', 'Is_REP', 'Date_Clean'])
        result = categorize_and_process_duplicates(single_samples, empty_bt)
        
        # Should return data unchanged when no duplicates exist
    ...
    def test_date_assignment_for_replicates(self):
        ...
        if len(red_river_samples) == 2:
            ...
            if 'collection_date_str' in red_river_samples.columns:
    ...
    def test_bt_data_matching_logic(self):
        """Test the BT data matching and year buffer logic."""
        # Create fish data for multiple years
        multi_year_fish = self.sample_fish_data.copy()
        multi_year_fish.loc[0:1, 'year'] = 2022  # Change Blue Creek to 2022
        
        # BT data for 2023 only
        result = categorize_and_process_duplicates(multi_year_fish, self.sample_bt_data)
        
        # Should still process both groups appropriately
        self.assertGreater(len(result), 0)
        
        # Blue Creek (2022 fish vs 2023 BT) should use year buffer
        blue_creek_samples = result[result['site_name'] == 'Blue Creek at Highway 9']
        red_river_samples = result[result['site_name'] == 'Red River at Bridge']
        
        # Both should be processed (exact year matching vs year buffer)
        self.assertGreater(len(blue_creek_samples), 0)
        self.assertGreater(len(red_river_samples), 0)
