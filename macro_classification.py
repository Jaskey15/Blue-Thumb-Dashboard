...
analysis_df = df[['comparison_to_reference', 'biological_condition']].copy()

# Remove any remaining invalid values (just in case)
analysis_df = analysis_df[analysis_df['biological_condition'].notna()]
...
print(f"Unique biological conditions: {sorted(analysis_df['biological_condition'].unique())}")
print(f"Score range: {analysis_df['comparison_to_reference'].min():.3f} to {analysis_df['comparison_to_reference'].max():.3f}")
print()

# 2. Count by biological condition
print("2. RECORDS BY BIOLOGICAL CONDITION:")
class_counts = analysis_df['biological_condition'].value_counts().sort_index()
for class_name, count in class_counts.items():
    print(f"  {class_name}: {count} records")
print()

# 3. Find boundaries for each class
print("3. SCORE BOUNDARIES BY CONDITION:")
for class_name in sorted(analysis_df['biological_condition'].unique()):
    class_data = analysis_df[analysis_df['biological_condition'] == class_name]['comparison_to_reference']
...
for idx, row in sorted_df.iterrows():
    current_class = row['biological_condition']
...
overlap_found = False
for class1 in sorted(analysis_df['biological_condition'].unique()):
    for class2 in sorted(analysis_df['biological_condition'].unique()):
        if class1 >= class2:  # Only check each pair once
            continue
            
        class1_data = analysis_df[analysis_df['biological_condition'] == class1]['comparison_to_reference']
        class2_data = analysis_df[analysis_df['biological_condition'] == class2]['comparison_to_reference']
